...
def calculates_results_stats(results_dic):
    ...
    for key, value in results_dic.items():
        # Check if the pet image is a dog image
        if value[3] == 1:
            n_dogs_img += 1
            # Check if the classifier label is a dog label
            if value[4] == 1:
                n_correct_dogs += 1
                # Check if the pet and classifier labels match
                if value[2] == 1:
                    n_match += 1
                    n_correct_breed += 1
        else:
            n_notdogs_img += 1
            # Check if the classifier label is not a dog label
            if value[4] == 0:
                n_correct_notdogs += 1
    ...
    # Add the calculated statistics to the results_stats_dic dictionary
